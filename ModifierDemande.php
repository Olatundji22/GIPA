...
if (isset($_GET["Dem"]))
{
	$requette = "SELECT * FROM demande WHERE NumDemande= '".$_GET["Dem"]."'";
		...
		<div class="container">
			<div class="col-md-5">
				...
				<?php $row1 = mysqli_fetch_array($resulta) ; ?>
					<form role="form" action="Demande.php" method="POST">
						<br style="clear:both">
						<h3 style="margin-bottom: 25px; text-align: center;">Modifier une Demande</h3>
						<div class="form-group">
							<input type="text" class="form-control" id="NumDem" name="NumDem" placeholder="Numéro de la Demande" value="<?php echo $row1["NumDemande"]; ?>" required>
						</div>
						<div class="form-group">
							<input type="hidden" class="form-control" id="Dem" name="Dem"  value="<?php echo $_GET["Dem"] ?>" required>
						</div>
						<select class="form-control" name="doc">
							<option value="<?php echo $row1["CodeDoc"]; ?>"><?php echo $row1["CodeDoc"]; ?></option>
							<?php while($row2 = mysqli_fetch_array($resultat1)):;?>
							<option value="<?php echo $row2[0]; ?>"><?php echo $row2[1]; ?></option>
							<?php endwhile; ?>
						</select><br>
						<select class="form-control" name="pers">
							<option value="<?php echo $row1["Matricule"]; ?>"><?php echo $row1["Matricule"]; ?></option>
