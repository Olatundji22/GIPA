...
if (isset($_GET["CodeT"]))
{
	$requette = "SELECT * FROM type_document WHERE CodeTypeDoc= '".$_GET["CodeT"]."'";
		...
		<div class="container">
			<div class="col-md-5">
				...
				<?php $row1 = mysqli_fetch_array($resulta) ; ?>
					<form role="form" action="Type de Document.php" method="POST">
						<br style="clear:both">
						<h3 style="margin-bottom: 25px; text-align: center;">Modifier un Type de Document</h3>
						<div class="form-group">
							<input type="text" class="form-control" id="CodeType" name="CodeType" placeholder="Code Type Document" value="<?php echo $row1["CodeTypeDoc"]; ?>" required>
						</div>
						<div class="form-group">
							<input type="text" class="form-control" id="LibelType" name="LibelType" placeholder="Nom Type Document" value="<?php echo $row1["LibelTypeDoc"]; ?>" required>
						</div>
						<div class="form-group">
							<input type="hidden" class="form-control" id="CodeT" name="CodeT"  value="<?php echo $_GET["CodeT"] ?>" required>
						</div>
            
						<button type="submit" name="Modifier" class="btn btn-primary pull-right">Modidfier</button>
					</form>
				<?php } ?>
					<?php
						if(isset($erreur))
