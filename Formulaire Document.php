...
{
	$CodeDoc = htmlspecialchars($_POST['CodeDoc']);
	$LibelDoc = htmlspecialchars($_POST['LibelDoc']);
	$FonctDoc = htmlspecialchars($_POST['FonctDoc']);
	$type = htmlspecialchars($_POST['type']);
	$file_name = htmlspecialchars($_POST['file_name']);
	$file_tmp_name = htmlspecialchars($_POST['file_tmp_name']);
	if(!empty($_POST) AND !empty($_FILES))
	{
		$file_name = $_FILES['fichier']['name'];
		$file_tmp_name = $_FILES['fichier']['tmp_name'];
		$file_dest = 'files/'.$file_name;
		$reqDoc = $bdd->prepare("SELECT * FROM document WHERE CodeDoc=?");
		$reqDoc->execute(array($CodeDoc));
		$CodeDocexist = $reqDoc->rowCount();
		if($CodeDocexist == 0)
		{
			if(move_uploaded_file($file_tmp_name, $file_dest))
			{
				$insertdoc=$bdd->prepare("INSERT INTO document(CodeDoc, LibelDoc, FonctionDoc, CodeTypeDoc, file_url) VALUES( ?, ?, ?, ?, ?)");
				$insertdoc->execute(array($CodeDoc, $LibelDoc, $FonctDoc, $type, $file_dest));
			...
			<div class="col-md-5">
				<div class="form-area">  
					<form role="form" action="#" method="POST" enctype="multipart/form-data">
						<br style="clear:both">
						<h3 style="margin-bottom: 25px; text-align: center;">Enregistrer un Document</h3>
						<div class="form-group">
							<input type="text" class="form-control" id="CodeDoc" name="CodeDoc" placeholder="Code Document" value="<?php if(isset($CodeDoc)) { echo $CodeDoc; } ?>" required>
						</div>
						<div class="form-group">
							<input type="text" class="form-control" id="LibelDoc" name="LibelDoc" placeholder="Libellé du Document" value="<?php if(isset($LibelDoc)) { echo $LibelDoc; } ?>" required>
						</div>
						<div class="form-group">
							<input type="text" class="form-control" id="FonctDoc" name="FonctDoc" placeholder="Fonction du Document" value="<?php if(isset($FonctDoc)) { echo $FonctDoc; } ?>" required>
						</div>
