...
{
	$NumDem = htmlspecialchars($_POST['NumDem']);
	$doc = htmlspecialchars($_POST['doc']);
	$pers = htmlspecialchars($_POST['pers']);
	if(!empty($_POST))
	{
		$DateDem = date(y-m-d);
		$reqDem = $bdd->prepare("SELECT * FROM demande WHERE NumDemande=?");
		$reqDem->execute(array($NumDem));
		$NumDemexist = $reqDem->rowCount();
		if($NumDemexist == 0)
		{
			$insertDem=$bdd->prepare("INSERT INTO demande(NumDemande, CodeDoc, Matricule) VALUES(?, ?, ?)");
			$insertDem->execute(array($NumDem, $doc, $pers));
			header('location: Demande.php');
			
		}
		else
		{
			$erreur = "Le numéro de la demande existe deja dans la base de donnés!!!";
		}
	}
}
...
$reqcombodoc = "SELECT * FROM document";
$resultat1 = mysqli_query($connect, $reqcombodoc);
$reqcombopers = "SELECT * FROM personnel";
$resultat2 = mysqli_query($connect, $reqcombopers);
					...
					<div class="collapse navbar-collapse" id="bs-navbar-collapse">
						<ul class="nav navbar-nav main-navbar-nav">
							...
							<li class="active" class="dropdown">
								<a href="Demande.php" title="" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">DEMANDE<span class="caret"></span></a>
								<ul class="dropdown-menu">
			...
			<div class="col-md-5">
				<div class="form-area">  
					<form role="form" action="#" method="POST">
						<br style="clear:both">
						<h3 style="margin-bottom: 25px; text-align: center;">Enregistrer une Demande</h3>
						<div class="form-group">
							<input type="text" class="form-control" id="NumDem" name="NumDem" placeholder="Numéro de la Demande" value="<?php if(isset($NumDem)) { echo $NumDem; } ?>" required>
						</div>
						...
						<select class="form-control" name="pers">
							<?php while($row1 = mysqli_fetch_array($resultat2)):;?>
							<option value="<?php echo $row1[0]; ?>"><?php echo $row1[1]; ?></option>
							<?php endwhile; ?>
						</select><br>
