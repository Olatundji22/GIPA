...
$reqdem = "SELECT NumDemande, DateDemande, LibelDoc, NomPerso, PrenPerso FROM demande, personnel, document WHERE demande.CodeDoc = document.CodeDoc AND demande.Matricule=personnel.Matricule ORDER BY DateDemande DESC";
$resultat = mysqli_query($connect, $reqdem);

if (isset($_POST["Modifier"]))
{
	$requete = "UPDATE demande SET NumDemande= '".$_POST["NumDem"]."' , CodeDoc= '".$_POST["doc"]."', Matricule= '".$_POST["pers"]."'  WHERE NumDemande= '".$_POST["Dem"]."' ";
   ...
   <body class="ola">
		...
		<div class="container">
			<div class="btn-toolbar">
				<a href="Formulaire Demande.php"><button class="btn btn-primary">Nouveau</button></a>
			</div><br>
			<div class="well">
				<table class="table">
					<thead>
					...
					<tbody>
						...
						<tr>
							<td><?php echo $row1["NumDemande"]; ?></td>
							<td><?php echo $row1[1]; ?></td>
							<td><?php echo $row1[2]; ?></td>
							<td><?php echo $row1[3]; ?></td>
							<td><?php echo $row1[4]; ?></td>
							<td><a href="ModifierDemande.php?Dem=<?= $row1["NumDemande"]?>"><button type="button" class="btn btn-warning">Modifier</button></a> 
							<a href="Supprimer.php?Dem=<?= $row1["NumDemande"]?>"><button type="button" class="btn btn-danger">Supprimer</button></a></td>
						</tr>
						<?php endwhile; ?>
